# soulnet-paygate-linux-arm64

The [soulnet](https://github.com/startupworld-ai/soulnet) local payment gateway binary for Linux ARM64 (`bin/paygate`), built with `go build -trimpath -ldflags "-s -w"` from `payment/cmd/paygate` of the same version.
