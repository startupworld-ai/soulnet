...
| `group.list` | — | `{groups[]}` | `groups[i] = {gid, name, owner_fp, mine, version, members, unread, count, last_ts?, last_body?, profile?, left?}` (`profile` lets UIs gate composers without fetching each group; `left: true` = the owner removed me or dissolved the group — the group is kept read-only with its history, `group.send` answers `-32602`, it flips back when the owner re-admits me) |
