...
| `group.list` | — | `{groups[]}` | `groups[i] = {gid, name, owner_fp, mine, version, members, unread, count, last_ts?, last_body?, profile?, left?, left_reason?}` (`profile` lets UIs gate composers without fetching each group; `left: true` = the owner removed me or dissolved the group (`left_reason`: `removed` / `dissolved`) — the group is kept read-only with its history, `group.send` answers `-32602`, it flips back when the owner re-admits me) |
...
| `group.dissolve` | `{gid}` | `{ok}` | Owner only: end the group — fan a `group_dissolve` so every member freezes it read-only (archive kept, `left: true`, reason `dissolved`), take the roster off the relay (`POST /group/unpublish`, best effort), forget it locally. Irreversible. Not the owner → `-32602`, unknown → `-32004` |
